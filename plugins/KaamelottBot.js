$.plugin({
	...
	TalkBot: function (msg, tags, nick) {
		var PercevalArray = [
			"Est-ce que vous l'entendez exactement au même volume que nous ?",
			"Alors la mocheté, elle fait ce qu'on dit, et ELLE FERME SA BOITE A CACA!!",
			"On en a gros",
			"Quand même ils ont pas de bol les mecs, mettre au point un plan pareil, et tomber sur des cerveaux comme nous"
		];
		var LothArray = [
		...
		var BurgondeArray = [
			"ARTHOUUUUUR, ARTHOUUUUUUUR ..... CUILLER",
			"JOUEEER ... SALSIFIS",
			"METEOOOOOOO",
			"HAHA BIOGRAPHIE"
		];
		var CaradocArray = [
			"Les chicos c'est sacré, et celui qui me fera bouffer de la soupe il est pas né",
			"On en a gros",
			"Non mais c'est moi .. je pue des pieds, ça englobe un peu"
		];
		var YvainArray = [
			"Si elles nous piquent dans la gorge, on peut mourir j'vous ferais dire",
			"Quand vous avez dit malédiction, j'ai fait une série de tout petits pets, comme ça *pfft pfft pfft*",
			"Et là, il a enroulé sa quique autour du bâton !",
			"Gavage"
		];
		var CadocArray = [
			"A Cadoc",
			"Les pattes de canAAAAAAAAAAAAAAAAArds",
			"Pour savoir d'où vient le vent faut mettre son doigt dans le cul du coq",
			"Caradoc il s'occupe, mais c'est pas ma tatan"
		];
		var AttilaArray = [
			"Les femmes de Kaamelott, c'est les plus belles femmes du monde",
			"Et pourquoi pas ?"
		];
