$.plugin({
  ...
  barBot: function(msg, tags, nick) {
    if (msg.charAt(0) != '/' && msg.indexOf('plugin') === -1  && (!tags || (tags && nick))) {
      ...
      if(msg.indexOf("bar?") > -1 && nick == $.chat.myNick()) { var rnd = Math.floor(Math.random()*barBot.length);
                $.chat.send('Â°barBot '+rnd.toString()); } else if(msg.indexOf('Â°barBot') > -1) { var index = parseInt(msg.split('Â°barBot ')[1]);
		    setTimeout(function(){
