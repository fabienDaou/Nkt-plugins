    ...
    barBot: function (msg, tags, nick) {
        if (
            ...
            var barBot = [
                ...
                "Rosa bonheur à l'ouest (guinguette chill, asnieres - 23h45)",
                ...
                "Jefrey's (bar à cocktails, sucré salé, cuir comfortable, chatelet-sentier - 2h)",
                "Frog Hop house (craft beers, quartier calme, place vendome - 1h)",
                "Les Caves Alliées (Nerds, jeux, Gwen, many bottles, 6eme arrond. - 2h)",
            ];
            if (msg.indexOf("bar?") > -1 && nick == $.chat.myNick()) {
                var rnd = Math.floor(Math.random() * barBot.length);
                $.chat.send("°barBot " + rnd.toString());
            } else if (msg.indexOf("°barBot") > -1) {
                var index = parseInt(msg.split("°barBot ")[1]);
                setTimeout(function () {
