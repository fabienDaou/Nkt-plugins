$.plugin({
  ...
  qrBot: function(msg, tags, nick) {
    if (msg.charAt(0) != '/' && msg.indexOf('plugin') === -1 && (!tags || (tags && nick))) {
      ...
      if(msg.indexOf("qrBot") > -1 && nick == $.chat.myNick()) { var rnd = Math.floor(Math.random()*QrArray.length);
                $.chat.send('°Qrquote '+rnd.toString()); } else if(msg.indexOf('°Qrquote') > -1) { var index = parseInt(msg.split('°Qrquote ')[1]);
		    setTimeout(function(){
